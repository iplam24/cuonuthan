<template>
  <header class="sticky top-0 z-40">
      ...
      <div class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3">
          ...
          <router-link
            ...
            to="/dang-nhap"
            class="hidden md:inline-flex items-center justify-center px-4 py-2 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200/70 hover:text-slate-900 transition"
          >
            Đăng nhập
          </router-link>
          ...
          <div v-else class="relative hidden md:block">
            <button
              @click="desktopAccountOpen = !desktopAccountOpen"
            ...
            <AccountFlyout
              :is-open="desktopAccountOpen"
              :loyalty="loyalty"
              @close="desktopAccountOpen = false"
            />
          </div>
    ...
    <!-- Mobile Account Bottom Sheet (Teleported to body) -->
    <Teleport to="body">
      <AccountMobileSheet
        v-if="authStore.isAuthenticated"
        :is-open="mobileAccountOpen"
        :loyalty="loyalty"
        @close="mobileAccountOpen = false"
      />
    </Teleport>

    <!-- Mobile Bottom Navigation Bar -->
    <nav class="fixed inset-x-0 bottom-0 z-40 grid h-16 grid-cols-4 border-t border-slate-200/80 bg-white/95 backdrop-blur-xl md:hidden shadow-lg" aria-label="Điều hướng di động">
      <!-- 1. Thực đơn -->
      <router-link
        to="/"
        class="flex flex-col items-center justify-center gap-1 text-[10px] font-semibold text-slate-500 hover:text-slate-900 transition-colors"
      ...
      <router-link
        to="/dat-mon"
        class="relative flex flex-col items-center justify-center gap-1 text-[10px] font-semibold text-slate-500 hover:text-slate-900 transition-colors"
        active-class="text-rose-600 font-bold"
      >
        <div class="relative">
          ...
          <span
            v-if="cartStore.itemCount"
            class="absolute -top-1.5 -right-2.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-600 px-1 text-[9px] font-extrabold text-white shadow-xs"
          >
            {{ cartStore.itemCount }}
          </span>
        </div>
      ...
      <!-- 4. Tài khoản (Khi đăng nhập: mở Bottom Sheet, Chưa: link tới /dang-nhap) -->
      <button
        v-if="authStore.isAuthenticated"
        type="button"
        class="relative flex flex-col items-center justify-center gap-1 text-[10px] transition-colors"
        :class="mobileAccountActive ? 'text-rose-600 font-bold' : 'text-slate-500 font-semibold hover:text-slate-900'"
        @click="mobileAccountOpen = !mobileAccountOpen"
      >
        <div class="relative">
          <User class="h-5 w-5" />
          <span
            v-if="loyalty?.points?.available"
            class="absolute -top-0.5 -right-1 w-2 h-2 rounded-full bg-amber-500 ring-2 ring-white"
          ></span>
        </div>
        <span>Tài khoản</span>
      </button>
...
import AccountMobileSheet from './AccountMobileSheet.vue';
...
const mobileAccountActive = computed(() => {
  return (
    mobileAccountOpen.value ||
    route.path.startsWith('/don-hang-cua-toi') ||
    route.path.startsWith('/tai-khoan')
  );
});
